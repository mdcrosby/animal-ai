...
def worker_aai(
    ...
    try:
        while True:
            er: EnvironmentRequest = parent_conn.recv()
            if er.cmd == EnvironmentCommand.STEP:
                all_action_info = er.payload
                ...
                reset_timers()
            elif er.cmd == EnvironmentCommand.EXTERNAL_BRAINS:
                _send_response("external_brains", external_brains())
            elif er.cmd == EnvironmentCommand.GET_PROPERTIES:
                reset_params = shared_float_properties.get_property_dict_copy()
                _send_response("get_properties", reset_params)
            elif er.cmd == EnvironmentCommand.RESET:
                env.reset(arenas_configurations=er.payload)
                all_step_result = _generate_all_results()
                _send_response("reset", all_step_result)
            elif er.cmd == EnvironmentCommand.CLOSE:
                break
